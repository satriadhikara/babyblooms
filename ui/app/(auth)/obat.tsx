...
const Obat = () => {
        ...
        try {
            ...
            if (result.success) {
                if (result.data && Array.isArray(result.data)) {
                    console.log(`Found ${result.data.length} medicines for letter ${letter}`);
                    setFilteredMedicines(result.data);
                } else {
                    console.log('Letter fetch successful but no data found or invalid format');
    ...
    const handleSearch = async () => {
        ...
        const normalizedSearchText = entryText.trim()
            .split(' ')
        ...
        try {
            const response = await fetch(`${API_URL}/medicine/search?query=${encodeURIComponent(normalizedSearchText)}`);
            const result = await response.json();
            ...
            if (result.success) {
                if (result.data && Array.isArray(result.data)) {
                    console.log(`Found ${result.data.length} medicines for query "${normalizedSearchText}"`);
                    setFilteredMedicines(result.data);
                } else {
    ...
    const renderMedicineItem = ({ item }: { item: Medicine }) => (
        <View style={{
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                <ThemedText type="titleMedium" style={{ flex: 1 }}>{item.name}</ThemedText>
    ...
    const shouldShowResults = selectedLetter !== null || searchPerformed || filteredMedicines.length > 0;
                ...
                >
                    ...
                    >
                        ...
                        >
                            ...
                            <ThemedText type="titleLarge" style={{ textAlign: 'center', marginBottom: 20 }}>
